...
void shuffleEvent(){
  for (int i = 1; i < 9; i++) {
    int randIndex = random(1, 9);
    int temp = event_list[i];
    event_list[i] = event_list[randIndex];
    event_list[randIndex] = temp;
  }
  // for (int i = 0; i<= 9; i++){
  //   Serial.print(event_list[i]);
  // }
  // Serial.println("");
}
